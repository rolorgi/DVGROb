...
def robustNerfMask(loss):
    epsilon = 1e-3
    error_per_pixel = torch.mean(loss, dim=-1, keepdim=True)  # mean loss per ray point, shape: [N,1]
    				       			      # dim=-1 is the rgb channel
    inlier_thr = torch.quantile(error_per_pixel, 0.5) # inlier vs outlier threshold based on quantile

    # 1) Filter inliers and outliers
    is_inlier_loss = (error_per_pixel < inlier_thr).float()  # shape: [N, 1]

    # 2) Diffuse losses to regularize weights
    window = torch.ones((1, 1, 3)) / (3 * 3)  # define the filter
    # apply convolution
    has_inlier_neighbors = F.conv1d(
    	torch.reshape(is_inlier_loss, (1,1,-1)), window,
    	stride=1, padding='same')
    # reshape to original dimension
    has_inlier_neighbors = torch.reshape(has_inlier_neighbors, (-1, 1))
    # binarize after smoothing
    has_inlier_neighbors = (has_inlier_neighbors >= 0.5).float()

    # 3) Filter each small patch iff its sorrounding is alredy filtered
    outer_patch_size = 16
    inner_patch_size = 8
    pad_size_lower = (outer_patch_size - inner_patch_size) // 2
    pad_size_upper = outer_patch_size - (inner_patch_size + pad_size_lower)
    inner_patch_mask = F.pad(
    	torch.ones(inner_patch_size),
    	(pad_size_lower, pad_size_upper))


    is_inlier_patch = torch.mean(is_inlier_loss, dim=-1, keepdim=True)  # f32[n,1]
    									# useless since we have 1D info (ray points not image pixels)
    									# but keep it for the names
    is_inlier_patch = (is_inlier_patch >= 0.6).float()
    n = int(is_inlier_patch.shape[0] / inner_patch_mask.shape[0])
    is_inlier_patch = is_inlier_patch * torch.t(inner_patch_mask.repeat(1, n))

    # A pixel is an inlier if it is an inlier according to any of the above criteria.
    mask = (is_inlier_patch + has_inlier_neighbors + is_inlier_loss > epsilon).float()
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        # Implemented RobustNeRF that applies mask to weights based on prev iter losses
        optimizer.zero_grad(set_to_none=True)
        loss = torch.mul(cfg_train.weight_main, torch.square(torch.sub(input=render_result['rgb_marched'], other=target)))
        
        if global_step > 1:
            mask = robustNerfMask(prev_loss)
            loss = torch.mul(mask, torch.square(torch.sub(input=render_result['rgb_marched'], other=target)))
        
        prev_loss = loss
        loss = torch.mean(loss)
	
	# Manually implemented because torch.nn.functional.mse_loss is not used anymore
        psnr = 20*torch.log(torch.max(render_result['rgb_marched'])) / torch.sqrt(loss)
